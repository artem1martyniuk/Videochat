...
import numpy as np
from process_audio import process_signal_realtime
from model_actual import RecurrentMaskNet
# from rnnoise_wrapper import RNNoise

model = torch.load("model_4gru.pth", map_location=torch.device("cpu"), weights_only=False)
model.eval()
# rnnoise = RNNoise()
relay = MediaRelay()
...
class AudioTransformTrack(MediaStreamTrack):
    ...
    async def recv(self):
        ...
        if len(self.buffer) >= self.buffer_size:
            ...
            processed_chunk = process_signal_realtime(audio_chunk)

            processed_audio = processed_chunk[:len(audio_data)]
            self.buffer = self.buffer[self.buffer_size:]

        processed_audio_int16 = (processed_audio * 32767.0).astype(np.int16)

        new_frame = AudioFrame(format='s16', layout='mono', samples=len(processed_audio_int16))
        new_frame.pts = frame.pts
        new_frame.sample_rate = frame.sample_rate
        new_frame.time_base = frame.time_base
        new_frame.planes[0].update(processed_audio_int16.tobytes())
